...
class CheckVserver(Check):
    ...
    async def run(asset: Asset, local_config: dict, config: dict) -> dict:
        ...
        return {
            'vserver': [{
                'aggregates_delegated': item.get(
                    'aggregates_delegated'),  # 9.7
                'anti_ransomware_default_volume_state':
                item.get('anti_ransomware_default_volume_state'),  # 9.10
                'certificate_uuid': item.get('certificate', {}).get(
                    'uuid'),  # 9.7
                ...
                item.get('snapshot_policy', {}).get('uuid'),
